...
InkTalk 是一款**语音优先**的 Android 输入法。点击即说、实时上屏，基于火山引擎「豆包流式语音识别大模型 2.0」，并提供手写、轻量数字键盘和可选全键盘；还可接入任意 OpenAI 兼容大模型完成自由语音指令、总结、翻译与整理。

默认仍使用轻量数字键盘。需要连续键入文字时，可在设置中启用全键盘，以同一入口输入中文拼音、英文、数字和符号。
...
- ⌨️ **可选全键盘** — 本地拼音候选、英文大小写、数字和常用符号共用一个输入面板
- 📋 **每日输入整理** — AI 自动生成当日输入摘要，高效回顾
- 🔥 **自适应热词** — 从每日记录逐字选择热词，并审核自动候选与可能纠错
- 🔒 **隐私安全** — 凭据只存本机；项目代码采用 MIT 协议，第三方资源保留原许可证

## 功能亮点

- 🎤 **中英混合与英文优先**：设置为“实时中英混合”时，中英文共用 `bigmodel_async`；设置为“英文优先定稿”时，可在中文与 English 间切换，English 使用 `bigmodel_nostream + en-US`。
- 🔢 **数字键盘或全键盘**：默认入口提供 `0–9`、`00`、小数点、正负号、冒号和斜杠。设置中启用「全键盘」后，同一位置改为键盘图标，并提供中文拼音候选、英文大小写、数字和常用符号；点击麦克风可返回语音输入。
...
- 📐 **可选极限高度**：默认语音模式可压缩到 `128dp`，保留顶部功能栏、麦克风与设置按钮；快捷键恢复 `240dp`，数字键盘、全键盘和手写恢复 `340dp`。窄屏中模式胶囊保留在左下角，空间足够时才移到顶部。
...
5. 在任意输入框唤出 InkTalk，选择中文或 English 后，点击大麦克风开始说话；再次点击结束。默认可点击麦克风左侧的拨号盘图标输入数字。需要完整键盘时，先在「输入法布局」中启用「全键盘」，再点击同一位置的键盘图标。
...
- 全键盘的中文输入使用随 APK 提供的本地无声调拼音词典，不需要联网。当前版本提供高频候选，不支持云端联想、模糊音和个性化词频学习。
...
项目代码基于 [MIT License](LICENSE) 开源。全键盘拼音词典来自 Android Open Source Project PinyinIME，并按 Apache License 2.0 使用；完整归属信息见 [THIRD_PARTY_NOTICES.md](THIRD_PARTY_NOTICES.md)。
